...
public:
	constexpr ALWAYS_INLINE _Basic___xor(value_type const (&str)[_length])
		: _Basic___xor(str, std::make_index_sequence<_length_minus_one>())
	{
...
private:
	...
	constexpr ALWAYS_INLINE _Basic___xor(value_type const (&str)[_length], std::index_sequence<indices...>)
...
using __xorA = class _Basic___xor<std::string, _length>;
template<size_t _length>
using __xorW = class _Basic___xor<std::wstring, _length>;
template<size_t _length>
using __xorU16 = class _Basic___xor<std::u16string, _length>;
template<size_t _length>
using __xorU32 = class _Basic___xor<std::u32string, _length>;
//---------------------------------------------------------------------------
template<typename _string_type, size_t _length, size_t _length2>
inline auto operator==(const _Basic___xor<_string_type, _length>& lhs, const _Basic___xor<_string_type, _length2>& rhs)
{
	static_assert(_length == _length2, "__xor== different length");

	return _length == _length2 && lhs.str() == rhs.str();
}
//---------------------------------------------------------------------------
template<typename _string_type, size_t _length>
inline auto operator==(const _string_type& lhs, const _Basic___xor<_string_type, _length>& rhs)
{
	return lhs.size() == _length && lhs == rhs.str();
}
//---------------------------------------------------------------------------
template<typename _stream_type, typename _string_type, size_t _length>
inline auto& operator<<(_stream_type& lhs, const _Basic___xor<_string_type, _length>& rhs)
{
	lhs << rhs.c_str();
