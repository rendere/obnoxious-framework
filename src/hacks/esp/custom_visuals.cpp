...
#define MV_HEALTH_TEXT __xor("90HP")
#define MV_ARMOR_TEXT __xor("90A")
#define MV_NAME_TEXT __xor("Name")
#define MV_WEAPON_TEXT __xor("M4A4")
#define MV_WEAPON_TEXT_ICO __xor("S") //M4A4 Icon
#define MV_DISTANCE_TEXT __xor("5m")
#define MV_MONEY_TEXT __xor("1000$")
#define MV_SCOPE_TEXT __xor("SCOPED")
#define MV_FLASHED_TEXT __xor("FLASHED")
#define MV_DEFUSING_TEXT __xor("DEFUSING")
...
void CEsp::CustomVisuals(Vec2 Start)
{
	...
	if (AmmoStyle == 0)
	{
		MV_AMMO_TEXT = __xor("[30]");
	}
	else if (AmmoStyle == 1)
	{
		MV_AMMO_TEXT = __xor("[30\\90]");
	}
	else if (AmmoStyle == 2)
	{
		MV_AMMO_TEXT = __xor("[120]");
	}
	...
	Vec2 StartPos = tahaGUI().GetCursorPos();

	Vec2 StartPosScreen = tahaGUI().GetCursorPos();
	...
	if (!VisualBodyImage)
		D3DXCreateTextureFromFileEx(GP_Render->m_pDevice, (CGlobal::SystemDisk + __xor("obnoxious\\Resources\\Images\\Body.png")).c_str(), 222, 579,
	...
	if (!MoveStruct::InitMItems)
	{
		...
		MV_Hp_Text->Name = __xor("MV_HP_Text");
		...
		MV_Armor_Text->Name = __xor("MV_Arm_Text");
		MV_Hp_Text->Color1 = Color(255, 255, 255, 255);
		...
		MV_Armor->Name = __xor("MV_ArmorBar");
		...
		MV_Weapon->Name = __xor("MV_Weapon");
		...
		MV_Distance->Name = __xor("MV_Distance");
		...
		MV_Flashed->Name = __xor("MV_Flashed");
		...
		MV_Defusing->Name = __xor("MV_Defusing");
	}
	...
	tahaGUI().SetCursorPos(Vec2(StartPos.x, StartPos.y + VIS_PREW_ZONE_Y + 3) - tahaGUI().GetThis()->Info.Pos,0);
}
