...
#include "../../Main.h"
#include "../../GUI/Gui.h"
#include "../../api/crypt/base64.h"
#include "../../api/crypt/crypt.h"
...
void CGHelper::AutoAddHelp(string _game_name)
{
	...
	if (CGlobal::LocalPlayer)
	{
		...
		for (int EntIndex = 0; EntIndex < I::EntityList()->GetHighestEntityIndex(); EntIndex++)
		{
			...
			if (string(pModelName).find(__xor("crosshair")) != string::npos)
			{
			...
			if (string(pModelName).find(__xor("*")) != string::npos)
			{
		...
		entry.act = __xor("throw");
		entry.descn = __xor("test_desc");
...
void CGHelper::AddHelp(string _game_name, GHInfo Info)
{
	...
	if (CGlobal::LocalPlayer)
	{
		...
		{
			Message::Get().Start(__xor("Already added!"));
			return;
		}
	...
	if (cur_map && CGlobal::LocalPlayer)
	{
		...
		for (size_t i = 0; i < cur_map->helpers.size(); i++)
		{
			...
			if (CGlobal::WorldToScreen(InfoPos, InfoPosScreen))
			{
				...
				if (selected)
					GP_Render->DrawString(16, Vec2(InfoPosScreen.x, InfoPosScreen.y - 40), Color::White(), false, true, __xor("Selected"));
			}
		...
		if (BestDistance < DistShowInfo)
		{
			...
			if (ShowDiscp)
			{
				string name = BestInfo.name + __xor(" (") + BestGName + __xor(")");
...
{
	std::ofstream o("C:\\obnoxious\\GrenadeHelper");
	nlohmann::json JO;
...
{
	string JCont = readFile("C:\\obnoxious\\GrenadeHelper");

	if (JCont == __xor("Read Error"))
		return false;
