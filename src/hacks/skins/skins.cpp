...
string pszDefaultCtModel = string(__xor("models/weapons/v_knife_default_ct.mdl"));
string pszDefaultTtModel = string(__xor("models/weapons/v_knife_default_t.mdl"));

string pszKnifeBayonet = string(__xor("models/weapons/v_knife_bayonet.mdl"));
string pszKnifeCSS = string(__xor("models/weapons/v_knife_css.mdl"));
string pszKnifeFlip = string(__xor("models/weapons/v_knife_flip.mdl"));
string pszKnifeGut = string(__xor("models/weapons/v_knife_gut.mdl"));
string pszKnifeKarambit = string(__xor("models/weapons/v_knife_karam.mdl"));
string pszKnifeM9Bay = string(__xor("models/weapons/v_knife_m9_bay.mdl"));
string pszKnifeHuntsman = string(__xor("models/weapons/v_knife_tactical.mdl"));
string pszKnifeFalchion = string(__xor("models/weapons/v_knife_falchion_advanced.mdl"));
string pszKnifeBowie = string(__xor("models/weapons/v_knife_survival_bowie.mdl"));
string pszKnifeButterfly = string(__xor("models/weapons/v_knife_butterfly.mdl"));
string pszKnifeShadow = string(__xor("models/weapons/v_knife_push.mdl"));
string pszKnifeCord = string(__xor("models/weapons/v_knife_cord.mdl"));
string pszKnifeCanis = string(__xor("models/weapons/v_knife_canis.mdl"));
string pszKnifeUrsus = string(__xor("models/weapons/v_knife_ursus.mdl"));
string pszKnifeNavaja = string(__xor("models/weapons/v_knife_gypsy_jackknife.mdl"));
string pszKnifeOutdoor = string(__xor("models/weapons/v_knife_outdoor.mdl"));
string pszKnifeStiletto = string(__xor("models/weapons/v_knife_stiletto.mdl"));
string pszKnifeTalon = string(__xor("models/weapons/v_knife_widowmaker.mdl"));
string pszKnifeSkeleton = string(__xor("models/weapons/v_knife_skeleton.mdl"));
...
void CSkins::UpdateSkins(bool reset)
{
	...
	ForceUpdated = reset;
	debug_log("ForceFullUpdate\n");
	CGlobal::FullUpdateCheck = false;
}

void CSkinListener::RegListener()
{
	I::GameEvent()->AddListener(this, __xor("player_death"), false);
	I::GameEvent()->AddListener(this, __xor("player_spawn"), false);
}
...
void CSkinListener::FireGameEvent(IGameEvent *event)
{
	if (GP_Skins)
	{
		...
		if (!strcmp(event->GetName(), __xor("player_spawn")))
		{
			if (I::Engine()->GetPlayerForUserID(event->GetInt(__xor("userid"))) == I::Engine()->GetLocalPlayer())
				GP_Skins->UpdateGlove = false;
		}

		if (!strcmp(event->GetName(), __xor("player_death")))
		{
			int nUserID = event->GetInt(__xor("attacker"));
			...
			const char* szWeapon = event->GetString(__xor("weapon"));

			for (auto ReplacementIcon : GP_Skins->g_KillIconCfg)
			{
				if (!strcmp(szWeapon, ReplacementIcon.first))
				{
					event->SetString(__xor("weapon"), ReplacementIcon.second);
					break;
				}
			}

			szWeapon = event->GetString(__xor("weapon"));
		}
		return;
	}
}
...
{
#define SV(o,a,b) j[__xor("Skins")][o][v.Name][/*__xor*/(a)] = b;
		...
		for (auto &v : It)
		{
			...
			if (SaveSticker)
			{
				for (int i(0); i < 5; i++)
				{
					...
					SV(sname, string(__xor("Stiker") + to_string(i) + __xor("kit")), v.Skin.Stickers[i].kit);
					SV(sname, string(__xor("Stiker") + to_string(i) + __xor("kit_menu_index")), v.Skin.Stickers[i].kit_menu_index);
					SV(sname, string(__xor("Stiker") + to_string(i) + __xor("rotation")), v.Skin.Stickers[i].rotation);
					SV(sname, string(__xor("Stiker") + to_string(i) + __xor("scale")), v.Skin.Stickers[i].scale);
					SV(sname, string(__xor("Stiker") + to_string(i) + __xor("wear")), v.Skin.Stickers[i].wear);
				}
			}
		}
	};

	SaveItem(__xor("Weapons"), WeaponNames, true);
	SaveItem(__xor("Knifes CT"), KnifeNames);
	SaveItem(__xor("Knifes TT"), KnifeNamesTT);
}

void CSkins::LoadSkins(nlohmann::json &j)
{
#define LV(o,a,b) if(!j[__xor("Skins")][o][v.Name][/*__xor*/(a)].is_null()) {b = j[__xor("Skins")][o][v.Name][/*__xor*/(a)];}
	...
	{
		if (!j[__xor("Skins")][sname].is_null())
		{
			for (auto& v : It)
			{
				if (!j[__xor("Skins")][sname][v.Name].is_null())
				{
					bool IsEmpty = false;
					...
					if (!j[__xor("Skins")][sname][v.Name][__xor("custom_name")].is_null())
					{
						string buf = j[__xor("Skins")][sname][v.Name][__xor("custom_name")].get<string>();
						for (int i(0); i < 32; i++)
					...
					if (LoadSticker)
					{
						for (int i(0); i < 5; i++)
						{
							...
							LV(sname, string(__xor("SRE") + to_string(i)), IsEmptyStick);
							if (IsEmptyStick)
							{
							...
							LV(sname, string(__xor("Stiker") + to_string(i) + __xor("kit")), v.Skin.Stickers[i].kit);
							LV(sname, string(__xor("Stiker") + to_string(i) + __xor("kit_menu_index")), v.Skin.Stickers[i].kit_menu_index);
							LV(sname, string(__xor("Stiker") + to_string(i) + __xor("rotation")), v.Skin.Stickers[i].rotation);
							LV(sname, string(__xor("Stiker") + to_string(i) + __xor("scale")), v.Skin.Stickers[i].scale);
							LV(sname, string(__xor("Stiker") + to_string(i) + __xor("wear")), v.Skin.Stickers[i].wear);
						}
					}
				}
			}
	...
	if (!j[__xor("Skins")].is_null())
	{
		LoadItem(__xor("Weapons"), WeaponNames, true);
		LoadItem(__xor("Knifes CT"), KnifeNames);
		LoadItem(__xor("Knifes TT"), KnifeNamesTT);
	}
}
...
void CSkins::ParseSortedKits()
{
	...
	string CsgoPatch = Utils::GetModuleBaseDir(0) + __xor("\\csgo");

	if (pSkins->Load(CsgoPatch, __xor("csgo")))
	{
		for (int iWeaponIndex = 0; iWeaponIndex < sizeof(pWeaponData) / sizeof(*pWeaponData); iWeaponIndex++)
		{
			string WeaponSearch = __xor("weapon_") + string(pWeaponData[iWeaponIndex]);
			...
			for (size_t iSkinWeaponIndex = 0; iSkinWeaponIndex < SkinInfoWeapon.size(); iSkinWeaponIndex++)
			{
				...
				string DragonKing = __xor("Dragon King");
		...
		DefKnifeEntry.name = __xor("Default");
		...
		{
			string KnifeSearch = __xor("weapon_") + string(pKnifeData[iKnifeIndex]);
	...
	delete pSkins;
	debug_log("Setup: Sorted Skins & Stickers sucessful\n");
}
...
void CSkins::PrepareSortedSkins()
{
	...
	SSEntry.sub_name = __xor("2019");
	SortedStickers.push_back(SSEntry);
	SSEntry.sub_name = __xor("2018");
	SortedStickers.push_back(SSEntry);
	SSEntry.sub_name = __xor("2017");
	SortedStickers.push_back(SSEntry);
	SSEntry.sub_name = __xor("2016");
	SortedStickers.push_back(SSEntry);
	SSEntry.sub_name = __xor("2015");
	SortedStickers.push_back(SSEntry);
	SSEntry.sub_name = __xor("2014");
	SortedStickers.push_back(SSEntry);
	SSEntry.sub_name = __xor("Other");
	SortedStickers.push_back(SSEntry);
	...
	for (size_t j(0); j < sticker_kits.size(); j++)
		if (sticker_kits[j].name.find(__xor("2019")) == string::npos &&
			sticker_kits[j].name.find(__xor("2018")) == string::npos &&
			sticker_kits[j].name.find(__xor("2017")) == string::npos &&
			sticker_kits[j].name.find(__xor("2016")) == string::npos &&
			sticker_kits[j].name.find(__xor("2015")) == string::npos &&
			sticker_kits[j].name.find(__xor("2014")) == string::npos)
			SortedStickers.back().Stickers.push_back(sticker_kits[j]);

	debug_log("Setup: All skins sorted sucessful\n");
}
