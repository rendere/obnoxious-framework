		...
		namespace Valve
		{
				...
				_end( 0 )
			{
				...
				for ( ++start; start < size; ++start )
                {
					...
					if (file->GetStringAt(start, 37) == __xor("SFUI_LoginPerfectWorld_AntiAddiction1"))
					{
						start += 600;
					}
                    else if ( p == '\"' )
					...
					else if ( qcount != 1 && qcount != 3 )
					{
						if ( p == '{' )
						{
							vpos[0] = file->FindFirstOf(__xor("\""), start , 0 ) - 1;
							vpos[1] = file->FindFirstOf(__xor("\""), vpos[0] , 0 );
			...
			{
				auto rstart = FindFirstOf(__xor("{"), 0, 200);
	...
	namespace Skins
	{
		...
		bool CSkins::DumpTable( File::Valve::mapLevel tab , File::Valve::mapVariable vt , File::Valve::mapLevel ai_wi ) {
			for ( auto& pk : tab )
			{
				...
				auto& pkname = pk.second->GetVariables().at(__xor("name"));
				auto& pkdesctag = string( pk.second->GetVariables().at(__xor("description_tag")));
				...
				for ( auto& wi : ai_wi )
				{
					auto& vip = wi.second->GetVariables().at(__xor("icon_path"));
					if ( vip[vip.size() - 7] != '_' )
						continue;
		...
		bool CSkins::Load( const string& gamePath , const string& gameShortName )
		{
			...
			if (!_items_game->Load(gamePath + __xor("/scripts/items/items_game.txt"), false))
				return false;

			_csgo_english = new File::Valve::CConfig();
			if (!_csgo_english->Load(gamePath + __xor("/resource/") + gameShortName + __xor("_english.txt"), true))
				return false;
			...
			// ughgughu
			auto& vt = fnFind( _csgo_english->GetLevel()->GetSubLevels() , __xor("Tokens"))->GetVariables();
			auto mhhh = fnFind( _items_game->GetLevel()->GetSubLevels() , __xor("alternate_icons2"));
			auto mhh = fnFind( mhhh->GetSubLevels() , __xor("weapon_icons"));
			auto& ai_wi = mhh->GetSubLevels();

			for ( auto& wat : _items_game->GetLevel()->GetSubLevels() )
			{
				if ( wat.first.find(__xor("paint_kits")) != string::npos )
				{
					// meh
