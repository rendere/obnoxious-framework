...
//tahaGUI().CheckBox("checkbb", &cb_test);

#define DCheckBox(text, var)  tahaGUI().CheckBox(__xor(text), &var); \
/*ImGui::Checkbox(text, &var); \
*/

#define DComboBox(text, var, items) tahaGUI().ComboBox(__xor(text), &var, items); \
/* ImGui::Combo(text, &var, items, IM_ARRAYSIZE(items));\
*/

#define SliderInts(text, var,  min_, max_)  tahaGUI().SliderInt(__xor(text), &var, min_, max_); \
/*ImGui::SliderInt(text, &var, min, max);\
*/	

#define SliderIntsProc(text, var,  min_, max_) tahaGUI().SliderIntProc(__xor(text), &var, min_, max_); \
/* ImGui::SliderIntProc(text, &var, min, max);\
*/


#define SliderFloats(text, var,  min_, max_) tahaGUI().SliderFloat(__xor(text), &var, min_, max_); \
/*ImGui::SliderFloat(text, &var, min, max);\
*/

#define HotsKey(text, var) tahaGUI().HotKey(__xor(text), &var); \
/* ImGui::Hotkey(text, &var); \
*/

#define DColorEdit(text, var) tahaGUI().ColorEdit2(__xor(text), &var); /*ImGui::MyColorEdit5(text, var##._color, var##.brainbow, 1 << 7 )*/
#define TextEdit(text, var, buf, sim) strcpy(buf, var.c_str()); tahaGUI().InputText(__xor(text), buf, sim); var = buf; /*strcpy(buf, var.c_str()); ImGui::InputText(text, buf, sim); var = buf;*/
#define TabsLabels(var, text, size, vertic) tahaGUI().TabLabels(&var, size, text, vertic);  /*ImGui::SubTabLabels(text, sizeof(text) / sizeof(text[0]), var, order);*/

#define lols(e) string(__xor(e))
#define lolc(e) __xor(e)
#define lolsc(e) string(__xor(e)).c_str()
