...
bool CGui::Begin(string Name, GuiWindowFlags Flags, Vec2 Size, Vec2 Pos, CGui* old, color_t* back)
{
	...
	if (tahaGUI().G_NextWindowPos.x != 0 && tahaGUI().G_NextWindowPos.y != 0)
	{
	...
	if ((tahaGUI().G_NextWindowPos.x == 0 &&
		tahaGUI().G_NextWindowPos.y == 0) &&
	...
	GP_Render->SetFont(tahaGUI().GP_Font);

	pThis->Info.ClipRect = WindowRect;

	pThis->Overlay.SetClipRect(Vec4(WindowRect.min, WindowRect.max));

	color_t back_color = tahaGUI().G_Style.clrBackground;

	if (back)
		back_color = *back;

	DrawFilledBox(WindowRect, back_color, tahaGUI().G_Style.wndRounding);

	pThis->Title_y = 0;
	Vec2 Padding = tahaGUI().G_Style.wndPadding;
	if (pThis->Info.Flags & GWF_Popup)
		Padding = tahaGUI().G_Style.itmInPadding;
	...
	if (!(pThis->Info.Flags & GWF_NoTitleBar) && !(pThis->Info.Flags & GWF_ChildWindow) && !(pThis->Info.Flags & GWF_Popup))
	{
		...
		DrawFilledBox(title_rect, tahaGUI().G_Style.clrTitle, tahaGUI().G_Style.wndRounding);
...
void CGui::End()
{
	...
	tahaGUI().G_PrevWindowPos = pThis->Info.Pos;
...
{
	int item_id = GetItemId(__xor("SCROLLY"));
	GuiIO &IO = tahaGUI().G_IO;
	GuiStyle &style = tahaGUI().G_Style;
	...
	if (tahaGUI().G_IO.ActivWindowName == pThis->Info.Name /*&& !press*/)
...
void CGui::Update()
{
	...
	for (auto it = tahaGUI().G_WindowList.begin(); it != tahaGUI().G_WindowList.end(); ++it)
		ResetOverlayGui(&it->second);

	tahaGUI().G_PopupDrawStack.clear();

	tahaGUI().ClosePopup = false;
}

bool CGui::WndProcHandler(HWND, UINT msg, WPARAM wParam, LPARAM lParam)
{
	GuiIO &IO = tahaGUI().G_IO;
	UINT button; // mouse4-mouse5 fix thanks taha
...
void CGui::ReleaseGui(CGui* pGui)
{
	...
	for (auto popup_it = pGui->PopupList.begin(); popup_it != pGui->PopupList.end(); ++popup_it)
		tahaGUI().G_PopupDrawStack.push_back(&popup_it->second);
...
void CGui::Cleanup() // ����������
{
	...
	if (tahaGUI().OpenedPopup != GetIO().ActivWindowName && tahaGUI().ClosePopup)
		GetIO().ActivWindowName = "";

	for (size_t i = 0; i < tahaGUI().G_PopupDrawStack.size(); i++)
		ReleaseGui(tahaGUI().G_PopupDrawStack[i]);
...
{
	return tahaGUI().G_PrevWindowPos;
}

void CGui::SetNextWindowPos(Vec2 Pos)
{
	tahaGUI().G_NextWindowPos = Pos;
}
...
{
	tahaGUI().GP_Font = pFont;
}

CFont* CGui::GetFont()
{
	return tahaGUI().GP_Font;
}

bool CGui::IsMouseHoverEx(Vec2 Min, Vec2 Max)
{
	Vec2 mouse = tahaGUI().G_IO.MousePos;
...
{
	string chc = tahaGUI().G_IO.ActivWindowName;
	
	if(!(pThis->Info.Flags & GWF_ChildWindow) && !(pThis->Info.Flags & GWF_Popup) 
		&& (tahaGUI().G_IO.ActivWindowName != "" &&
		tahaGUI().G_IO.ActivWindowName != pThis->Info.Name))
		return false;
...
void CGui::ShowMetrics()
{
	Begin(__xor("Metrics"));
	{
		...
		//IO.G_DeltaTime
		Text(__xor("DeltaTime: %f"), GetIO().G_DeltaTime);
		for (auto it = tahaGUI().G_WindowList.begin(); it != tahaGUI().G_WindowList.end(); ++it)
		{
			Text(__xor("%i: Window name: %s Pos(%i, %i) Size(%i, %i)"),
			...
			Text(__xor("\t VTX: %i"), it->second.Overlay.VtxBuffer.size());
			Text(__xor("\t IDX: %i"), it->second.Overlay.IdxBuffer.size());
			Text(__xor("\t CMD: %i"), it->second.Overlay.CmdBuffer.size());
			for (int i(0); i < it->second.Overlay.CmdBuffer.size(); i++)
			{
				Vec4 rec = it->second.Overlay.CmdBuffer[i].ClipRect;
				Text(__xor("\t\t Size: %f, %f, %f, %f"), rec.x, rec.y, rec.z, rec.w);
			}
			Text(__xor("\t ActivID: %X"), it->second.ActivItemId);

			idx++;
		}
		Text(__xor("Total VTX: %i"), GP_Render->TotalVtxCount);
		Text(__xor("Total IDX: %i"), GP_Render->TotalIdxCount);


		Text(" ");
		Text(__xor("MousePos X: %i, Y: %i"), (int)tahaGUI().G_IO.MousePos.x, (int)tahaGUI().G_IO.MousePos.y);
		Text(__xor("LeftMouse: %i"), (int)tahaGUI().G_IO.LMPress);
		Text(__xor("tahaGUI().G_IO.ActivWindowName: %s"), tahaGUI().G_IO.ActivWindowName.c_str());

		//tahaGUI().G_IO.ActivWindowName

		static int ct = 0;
		if (tahaGUI().G_IO.LMClicked)
			ct++;

		Text(__xor("LeftMouseClick: %i (%i)"), (int)tahaGUI().G_IO.LMClicked, ct);
	}
	End();
}
...
void CGui::TextFmt(Vec2 Pos, const char* label, ...)
{
	...
	DrawString(Pos.x, Pos.y, tahaGUI().G_Style.clrText, false, false, buf);
}
void CGui::Text(Vec2 Pos, const char* label)
{
	DrawString(Pos.x, Pos.y, tahaGUI().G_Style.clrText, false, false, label);
}
...
void CGui::Text(const char* label, ...)
{
	...
	SetCursorPos(Vec2(tahaGUI().G_Style.wndPadding.x, GetCursorPosEx().y + size.y + tahaGUI().G_Style.itmPadding.y), 
		size.x + tahaGUI().G_Style.itmPadding.x);
...
bool CGui::ButtonBehavior(Rect Rect, bool &Hovered)
{
	...
	Hovered = (IsMouseHover(Rect.min, Rect.max) && pThis->ActivItemId == -1) ||
		(pThis->ActivItemId == pThis->ItemsId && tahaGUI().G_IO.LMPress);

	if (Hovered)
		pThis->ActivItemId = pThis->ItemsId;
	if ((!Hovered && pThis->ActivItemId == pThis->ItemsId) ||
		((pThis->ActivItemId == pThis->ItemsId || pThis->ActivItemId == -1) && !tahaGUI().G_IO.LMPress))
		pThis->ActivItemId = -1;

	if (Hovered)
		pThis->LastItemHovered = true;

	if (pThis->ItemsId == pThis->ActivItemId && Hovered && tahaGUI().G_IO.LMClicked)
	{
...
bool CGui::Button(string Label, Vec2 Size)
{
	...
	SetCursorPos(Vec2(style.wndPadding.x, GetCursorPosEx().y + button_size.y + style.itmPadding.y),
		button_size.x + tahaGUI().G_Style.itmPadding.x);
	...
	if (hovered && tahaGUI().G_IO.LMPress)
...
bool CGui::CheckBox(string Label, bool *var)
{
	...
	SetCursorPos(Vec2(style.wndPadding.x, GetCursorPosEx().y + cb_size.y + style.itmPadding.y),
		cb_size.x + label_size.x + style.itmInPadding.x + 3 + tahaGUI().G_Style.itmPadding.x);
...
bool CGui::SliderFloat(string Label, float *var, float min, float max, const char* display_format)
{
	...
	bool var_change = hovered && tahaGUI().G_IO.LMPress;
...
{
	float buf = (float)(*var);
	bool change = SliderFloat(Label, &buf, (float)min, (float)max, __xor("%6.0lf"));
	*var = (int)buf;
...
{
	float buf = (float)(*var);
	bool change = SliderFloat(Label, &buf, (float)min, (float)max, __xor("%.0f%%"));
	*var = (int)buf;
...
{
	tahaGUI().OpenedPopup = popup;
	GetIO().ActivWindowName = popup;
}
bool CGui::IsPopupOpen(string popup)
{
	return tahaGUI().OpenedPopup == popup;
}
...
bool CGui::SelectLabel(string Label, bool Selected, Vec2 Size)
{
	...
	bool hovered = false;
	bool clicked = ButtonBehavior(button_rect, hovered);
	bool press = tahaGUI().G_IO.LMPress;
...
bool CGui::ComboBox(string Label, int *var, VectorEx<const char*> &items)
{
	...
	if ((IsPopupOpen(Label) || style.AnimationEnable && popup_y > 0) && !CheckSkip(popup_rect))
	{
		...
		if (((!IsMouseHoverEx(popup_rect.min, popup_rect.max) && IO.LMClicked) || close_popup) && IsPopupOpen(Label))
		{
			...
			tahaGUI().ActivItemId = -1;
			tahaGUI().ClosePopup = true;
			IO.ActivWindowName = "";
			IO.LMClicked = false;
			IO.LMPress = false;
		}
...
bool CGui::ColorEdit2(string Label, SDK::Color *var, Vec2 Size)
{
	...
	if (IsPopupOpen(Label))
	{
		BeginIntenrnalWin(Label, GWF_Popup, popup_size, popup_rect.min, style.clrFrame);
		{
			...
			Separator();
			Spacing();
			ColorEdit(Label + __xor("##pick_2"), var, Vec2(popup_size.x - (style.itmInPadding.x*2) , 0));
			Spacing();
			Separator();
			Spacing();
			CheckBox(__xor("Rainbow"), &var->brainbow);
		}
		EndIntenrnalWin();
...
bool CGui::ColorEdit(string Label, float *var, Vec2 Size)
{
	...
	SetCursorPos(Vec2(tahaGUI().G_Style.wndPadding.x, GetCursorPosEx().y + full_frame_size.y + tahaGUI().G_Style.itmPadding.y),
	...
	bool changed = button_check(r_rect, var[0], __xor("R:"));
	changed = button_check(g_rect, var[1], __xor("G:"));
	changed = button_check(b_rect, var[2], __xor("B:"));
...
bool CGui::ColorPicker(string Label, float *var)
{
	...
	SetCursorPos(Vec2(tahaGUI().G_Style.wndPadding.x, GetCursorPosEx().y + color_matrix_size.y + tahaGUI().G_Style.itmPadding.y),
...
void CGui::SameLine(float len)
{
	if (len == 0)
		...
		SetCursorPos(Vec2(tahaGUI().G_Style.wndPadding.x + len, pThis->PrevCursorPos.y),0);
}
...
void CGui::Separator()
{
	...
	Vec2 line_start = Vec2(window_pos.x,(int)(pos.y + (tahaGUI().G_Style.itmPadding.y / 2)-2));
	Vec2 line_end = Vec2(line_start.x + window_size.x, line_start.y);

	DrawLine(line_start, line_end, tahaGUI().G_Style.clrLine);

	SetCursorPos(Vec2(tahaGUI().G_Style.wndPadding.x, GetCursorPosEx().y + tahaGUI().G_Style.itmPadding.y ),0);
}
...
void CGui::Spacing()
{
	SetCursorPos(Vec2(pThis->CursorPos.x, pThis->CursorPos.y + (tahaGUI().G_Style.itmPadding.y/2)), 0);
}
...
{
	DWORD id = GetItemId(__xor("##Labels"));
	Vec2 pos = GetCursorPos();
	GuiStyle &style = tahaGUI().G_Style;
	GuiIO &IO = tahaGUI().G_IO;
...
{   "Unknown","LBUTTON","RBUTTON","CANCEL","MBUTTON","MOUSE4","MOUSE5","Unknown","BACK","TAB","Unknown","Unknown","CLEAR","RETURN","Unknown","Unknown","SHIFT",
...
bool CGui::HotKey(string Label, int *var)
{
	...
	bool hovered = false;
	bool clicked = ButtonBehavior(frame_rect, hovered);
	bool pressed = tahaGUI().G_IO.LMPress;
	...
	if (pThis->UsingItemId == id /*&& !clicked*/ /*&& !clicked && !hovered*/)
	{
		...
		*var = key;
		strcpy(buf_display, __xor("<Press a key>"));
	}
...
{
	tahaGUI().DrawFilledBox(Rect(a,  a + b), color, rounding);
}
void CGui::DrawFilledBoxEx(Vec2 a, Vec2 b, color_t color, float rounding)
{
	tahaGUI().DrawFilledBox(Rect(a, b), color, rounding);
}
...
{
	tahaGUI().DrawBox(Rect(a, b),  1, color, rounding);
}

void CGui::DrawBox(int16_t x, int16_t y, int16_t width, int16_t height, int16_t thickness, color_t color, float rounding)
{
	tahaGUI().DrawBox(Rect(Vec2(x, y), Vec2(x + width, y + height)), thickness, color, rounding);
}
...
void CAnimController::Animator100Ex(float &Current, const float SpeedCor1, const float Delta)
{
	...
	if (Current < 100.f)
	{
		...
		float cor =  ((1000 / tahaGUI().GetIO().G_Framerate) / 100.f) / (Speed);
